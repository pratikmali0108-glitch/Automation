...
def _get_obu_cs():
    ...
    try:
        ...
        print(f"[!] Warning: Failed to load OBU_CS.xlsx: {e}")
        _OBU_CS_CACHE = pd.DataFrame()
    return _OBU_CS_CACHE
...
def _infer_style_suffix_pattern(style_no):
    ...
    pattern = 'LEGACY'
    df = _get_obu_cs()
    if df is not None and isinstance(df, pd.DataFrame) and not df.empty and 'Style No' in df.columns:
    ...
    try:
        ...
        if ext == '.pdf':
            ...
            for b_index, block in enumerate(blocks):
                ...
                sku_no = sku_full if sku_full else ''
...
def main():
    ...
    print(f"[*] Processing OBU data from PDF: {pdf_path}")
    
    # Run the OBU parser logic
    success, excel_path, error, df = process_obu_file(pdf_path, output_dir)
    ...
    print("[*] Starting UI display sequence in 2 seconds...")
...
# -------------------------------------------------------------
    # Step 2: Open Generated Excel Output and Force Snap to RIGHT Half
    # -------------------------------------------------------------
    print("[*] Opening generated Excel file on the RIGHT side...")
    os.startfile(excel_path)
    
    # Increase load wait time slightly for Excel initialization
    time.sleep(3.5)

    # Bring Excel window to focus reliably before sending Snap hotkey
    try:
        import pygetwindow as gw
        # Find Excel window by file name or application title
        excel_title = os.path.basename(excel_path)
        windows = [w for w in gw.getAllWindows() if 'Excel' in w.title or excel_title in w.title]
        if windows:
            win = windows[0]
            if win.isMinimized:
                win.restore()
        ...
        # Fallback to coordinate click if pygetwindow is not installed
        print(f"[!] pygetwindow fallback: clicking target region for focus...")
        excel_ribbon_x = int(screen_width * 0.75)
        excel_ribbon_y = int(screen_height * 0.05)  # Moved higher onto the title bar
        pyautogui.click(excel_ribbon_x, excel_ribbon_y)
        time.sleep(0.5)

    # Snap Excel to RIGHT half
    pyautogui.hotkey('win', 'right')
    time.sleep(0.5)
    pyautogui.press('escape')  # Dismiss Snap Assist menu grid
    time.sleep(0.5)
    # -------------------------------------------------------------
# -------------------------------------------------------------
    # Step 3: Open SJE PLUS Software
    # -------------------------------------------------------------
    print("[*] Opening SJE PLUS via Windows Search Bar...")
    ...
    # Give Windows OS 2 seconds to load the search UI completely
    print("[*] Waiting 2 seconds for OS Search Bar to load...")
    time.sleep(2.0)

    # Type search term and trigger application launch
    human_type("sje plus")
    time.sleep(0.5)
    pyautogui.press('enter')

    # -------------------------------------------------------------
    # Step 4: Wait for User Login Credentials (2x Enter key press)
    # -------------------------------------------------------------
    print("\n[!] PAUSED: Please enter credentials in SJE PLUS and press ENTER TWICE when logged in...")
    
    # Wait for first Enter key press
    keyboard.wait('enter')
    time.sleep(0.3)
    # Wait for second Enter key press
    keyboard.wait('enter')
    
    print("[*] Detected dual Enter presses! Continuing automation...")
    time.sleep(1.5)
    ...
    # -------------------------------------------------------------
    # Step 5: Hover to "Type here to search Menu", Type, Down Arrow & Enter
    # -------------------------------------------------------------
    print("[*] Hovering to SJE PLUS Search Bar...")
    
    # Coordinates mapped from SJE PLUS layout
    menu_search_x = int(screen_width * 0.515)
    menu_search_y = int(screen_height * 0.025)

    human_move_to(menu_search_x, menu_search_y, duration=0.8)
    pyautogui.click()
    time.sleep(0.3)

    print("[*] Typing 'commerical order'...")
    human_type("commerical order")
    time.sleep(0.3)

    print("[*] Pressing Down Arrow and then Enter...")
